...
docker run --link mysql-redmine:mysql --rm --env RAILS_ENV=production ir2sora/redmine rake db:create
docker run --link mysql-redmine:mysql --rm --env RAILS_ENV=production ir2sora/redmine rake db:migrate
docker run --link mysql-redmine:mysql --rm --env RAILS_ENV=production --env REDMINE_LANG=ru ir2sora/redmine rake redmine:load_default_data
